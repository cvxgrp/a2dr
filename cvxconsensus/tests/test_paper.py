...
class TestPaper(BaseTest):
    ...
    def test_nnls(self):
        ...
        # Problem data.
        m, n = 10000, 8000  #10000, 10000 #2000, 10000 #5000, 10000 #1000, 10000 #10000, 8000 #100, 1000
        density = 0.001
...
#         # Solve with SciPy.
#         sp_result = nnls(sparse.csr_matrix(X).todense(), y)
#         sp_beta = sp_result[0]
...
#         ## debug
#         prox_list_copy = copy.deepcopy(prox_list)
#         A_list_copy = copy.deepcopy(A_list)
#         b_copy = copy.deepcopy(b)
        
        # Solve with DRS.
        drs_result = a2dr(prox_list, A_list, b, t_init = 1, anderson=False, precond=True, max_iter=self.MAX_ITER)
        #print('YYY3')
...
#         ## debug
#         print([prox_list_copy[i] == prox_list[i] for i in range(len(prox_list))])
#         print([A_list_copy[i].todense() == A_list[i].todense() for i in range(len(A_list))])
#         print(b_copy == b)
...
#         self.assertAlmostEqual(sp_obj, a2dr_obj)
    ...
    def test_nnls_reg(self):
        ...
        # Problem data.
        m, n = 1000, 10000 #5000, 10000 #1000, 10000 #10000, 8000 #100, 1000
        density = 0.001
#         beta_true = np.array(np.arange(-n/2,n/2) + 1)
        X = sparse.random(m, n, density=density, data_rvs=np.random.randn)
        y = np.random.randn(m)
        ...
        # 1e-14, and tol = 1e-16 another good example
        # another good example: 1e-10 if lsqr tol = 1e-10 or so: match the accuracy of the subsolvers -- otherwise, it will go so closely to the no-reg one, which is not very stable with large errors
...
#         v_init = 2*[S_init.ravel(order='C')]

        # Solve with DRS.
        drs_result = a2dr(prox_list, A_list, b, anderson=False, precond=True)#, eps_abs=1e-9, eps_rel=1e-9)#, max_iter=3000)
        ...
        # Solve with A2DR.
        a2dr_result = a2dr(prox_list, A_list, b, anderson=True, precond=True)#, eps_abs=1e-9, eps_rel=1e-9)#, lam_accel=1e-10, max_iter=3000)
        a2dr_S = a2dr_result["x_vals"][0].reshape((n,n), order='C')
        a2dr_obj = -LA.slogdet(a2dr_S)[1] + np.sum(np.diag(a2dr_S.dot(Q))) + alpha*np.sum(np.abs(a2dr_S))
#         cvxpy_obj_raw = -LA.slogdet(cvxpy_S)[1] + np.sum(np.diag(cvxpy_S.dot(Q))) + alpha*np.sum(np.abs(cvxpy_S))
...
#         self.assertAlmostEqual(cvxpy_obj_raw, a2dr_obj)
